...
fig, ax = plt.subplots(figsize=(12, 4))
x = train_data.head(30)['datetime'].to_numpy()
y = train_data.head(30)['count'].to_numpy()
...
if submit_button:
    # ✅ FIXED multi-dimensional indexing error
    input_data = np.array([temp, atemp, humidity, windspeed, hour, dayofweek]).reshape(1, -1)
    prediction = model.predict(input_data)[0]
    st.metric("🚲 Predicted Bike Rentals", f"{int(prediction)}")
